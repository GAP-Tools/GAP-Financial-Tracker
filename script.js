        ...
        categoryContainer.innerHTML = `
            <td colspan="5">
                <table class="category-table">
                    ...
                    <tbody id="category-body-${monthIndex}">
                    </tbody>
                </table>
            </td>
        `;
            ...
            dailyContainer.innerHTML = `
                <td colspan="4">
                    <table class="daily-table">
                        ...
                        <tbody id="daily-body-${monthIndex}-${catIndex}">
                        </tbody>
                    </table>
                </td>
            `;
...
function generateStory() {
    ...
    let story = `Meet ${profile.name}, a ${profile.age}-year-old ${profile.occupation} with a dream to ${profile.dream}. Currently, ${profile.name} earns an average of ${profile.currency} ${totalIncome} per month but spends ${profile.currency} ${totalExpenses}, leaving an average cashflow of ${profile.currency} ${totalIncome - totalExpenses}. They own assets worth ${profile.currency} ${totalAssets} and have liabilities of ${profile.currency} ${totalLiabilities}, resulting in a net worth of ${profile.currency} ${netWorth}. Their goal is to achieve a passive income of ${profile.currency} ${profile.passiveIncomeTarget}.`;
    profile.fundAllocations.categories.forEach(cat => {
        story += ` ${cat.name} currently has a balance of ${profile.currency} ${cat.balance} with ${cat.transactions.length} transactions. `;
    });
